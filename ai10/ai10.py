...
def preprocess_image(image_path):
    ...
    image_resized = image_resized.reshape(1, 64, 64, 1)  # Reshape for the model
    return image_resized

def predict_image(image_path):
    image = preprocess_image(image_path)
    prediction = np.argmax(model.predict(image))
    return shape_types[prediction]
...
image_path = 'test.png'
print(f'Prediction for the image: {predict_image(image_path)}')
